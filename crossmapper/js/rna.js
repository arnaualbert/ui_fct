...
function sendRna_v2() {
    ...
    document.getElementById('command').innerHTML = `crossmapper RNA -g ${fastq_ls_string} -gn ${genome_name_string} -rlen ${read_length_string} -rlay ${readConfiguration.value} -N ${number_of_reads_string} -a ${annotations_gtf_ls_string} -t ${numberofcores.value} -e ${baseerorrate.value} -d ${outerDistance.value} -s ${standarDeviation.value} -C ${coverage.value} -r ${mutationRate.value} -R ${indelFraction.value} -X ${indelExtended.value} -S ${seedRandomGenerator.value} -AMB ${discarambiguous.value} -hapl ${haplotypeMode.value} -o ${outputDirectory.value} --verbose ${verboseMode.value} -gb ${groupBarChart.value} -rc ${reportCrossMapped.value} --mapper-template ${mapperTemplatePath.value} -max_mismatch_per_len ${maxMissmatchPerLen.value} -bact_mode ${bacterialMode.value} -max_mismatch ${maxMissmatch.value} -star_tmp ${starTmp.value}`
